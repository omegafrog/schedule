...
void fcfs_scheduling(process* processList, unsigned int size, Result* resultList) {	//자료형에 unsigned가 붙으면 0이상의 값만 표현함.
	...
	int queue_start_time = 0;	//queue의 시작시간을 구하기 위한 변수
	QueueType queue;
	int element;
	int count = 0;
	...
	for (int i = 0; i < size; i++) {
		...
		/*type이 0 입력될 경우*/
		if (newProcessList[i].type == 0) {	//type이 0 이라면 1) ready queue에 들어가자! 2) queue에 들어온 도착시간을 구하자
			...
			element = saveprocessList[count].process_id;	//pid를 element에 대입하자
			enqueue(&queue, element);	//레디큐에 삽입하자

		

			/*도착시간 구하기*/
			if (scheduling_time == 0) {	//type이 0이면 초기 도착시간은 0 
				arrive_time[count] = 0;
			}	
			else {
			...
			/*시작시간 구하기*/
			if (element == 1) { start_time[count] = 0; }	//만약 첫 번째 프로세스이면 레디 큐 내의 시작 시간은 0이된다.
		...
		/*type이 1 입력될 경우*/
		if (newProcessList[i].type == 1) {	//type이 1 이라면 1) ready queue 삭제하기 2)작업을 마쳤으므로 resultList에 저장		
			element = dequeue(&queue);	//레디 큐에 맨 앞에 있는 친구 삭제 후 프로세스 값을 리턴 받는다.
		...
		//queue_start_time += 1;	//시작시간 1증가한다.

		/*type이 -1 입력될 경우*/
		if (newProcessList[i].type == -1) {	//입력완료 되었다는 뜻이므로 순서대로 레디큐에서 작업 실행
			break;
	...
	while (!is_empty(&queue)) {	//큐가 빌 때까지 계속 스케줄링
		element = dequeue(&queue);	//레디 큐에 맨 앞에 있는 친구 삭제 후 프로세스 값을 리턴 받는다.
